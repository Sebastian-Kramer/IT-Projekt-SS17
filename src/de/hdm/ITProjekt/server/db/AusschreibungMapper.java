		...
		public Vector<Ausschreibung> getAll(){
			 ...
			 Vector<Ausschreibung> result = new Vector<Ausschreibung>();
			  ...
			  try {
			      ...
			      ResultSet rs = stmt.executeQuery("SELECT ID, ausschreibungstext, bezeichnung, datum, Projekt_ID, Orga_ID, Projektleiter_ID FROM Ausschreibung");
			  ...
			  while (rs.next()) {
					...
					p.setProjekt_ID(rs.getInt("Projekt_ID"));
					p.setOrga_ID(rs.getInt("Orga_ID"));
					p.setPartnerprofil_ID(rs.getInt("Projektleiter_ID"));
